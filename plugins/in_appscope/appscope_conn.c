...
int appscope_conn_event(void *data)
{
    ...
    if (event->mask & MK_EVENT_READ) {
        ...
        // TODO REMOVE THIS
        flb_plg_debug(ctx->ins, "appscope_conn_event fd=%i incoming data exceed limit (%zd bytes)", event->fd, (ctx->buffer_max_size) );
        flb_plg_debug(ctx->ins, "appscope_conn_event %d available", available );
        if (available < 1) {
...
/* Create a new Appscope request instance */
struct appscope_conn *appscope_conn_add(int fd, struct flb_appscope *ctx)
{
    ...
    //TODO REMOVE
    flb_plg_debug(ctx->ins, "appscope_conn_add");
    return conn;
}
